import { TEAMS_BASE, GROUPS } from "./data.js";
  ...
  matches.forEach((match) => {
    ...
    if (editable && state.canEdit) {
      ...
      inputs[0].addEventListener("change", (e) => {
        const newHomeGoals = e.target.value === "" ? null : Number(e.target.value);
        const newAwayGoals =
      ...
      inputs[1].addEventListener("change", (e) => {
        ...
        const newAwayGoals = e.target.value === "" ? null : Number(e.target.value);
...
function renderOwnerView(state, owner, onMatchChange) {
  ...
  const teams = getOwnedTeams(owner).sort((a, b) => {
